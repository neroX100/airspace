...
export default function ProductCard({id,image,title,price,address,description,bathroom,bedroom,restroom,}) {
  return (
    <Link to={`/listing/${id}`} className="rounded-md overflow-hidden min-h-44 md:min-h-56 hover:-translate-y-1 hover:shad bg-white">
                    <div className="h-32 w-full overflow overflow-hidden relative">
                        <div className="bg-slate-700 py-1 px-4 w-32 rounded-2xl text-xs sm:text-sm text-white text-center absolute z-10 top-2 left-3">&#8358; {price.toLocaleString()}</div>
